<template>
  <div class="roles">
    ...
    <el-dialog
      ...
      width="50%"
      @close="resetForm($refs.addRoleFormRef)">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm($refs.addRoleFormRef, 'message')">重 置</el-button>
    ...
    <el-dialog
      ...
      width="50%"
      @close="resetForm($refs.editRoleFormRef)">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm($refs.editRoleFormRef, 'message')">重 置</el-button>
...
<script>
import {getRolesList, addRole, searchRoleByRoleId, editRole, removeRole, removeRoleRight, getRightsList, setRights} 
from 'network/rights'
...
export default {
  ...
  data() { 
    return {
      ...
      // 验证角色添加的规则对象
      addFormRules: {
        roleName: [requiredRule(`请输入角色名称`), lengthRule(2, 15)],
        roleDesc: [requiredRule(`请输入角色描述`), lengthRule(2, 50)],
      },
    }
  },
  created () {
  ...
  methods: {
    ...
    addRole() {
      this.$refs.addRoleFormRef.validate(async (valid) => {
        ...
        if(!data) return
        if(data.meta.status !== 200) return this.$message.error('添加角色失败')
    ...
    async showEdit(id) {
       ...
       const{data} = await searchRoleByRoleId(id)
       if(!data) return
       if(data.meta.status !== 200) {
      ...
      //  给编辑角色对象赋值
      this.editForm.roleName = data.data.result.roleName
      this.editForm.roleDesc = data.data.result.roleDesc
      this.editForm.role_id = data.data.result.role_id
    },
    // 修改角色
    editRole() {
      this.$refs.editRoleFormRef.validate(async (valid) => {
        if(!valid) return
        const {data} = await editRole(this.editForm.role_id, this.editForm)
        if(!data) return
    ...
    async removeRole(id) {
      ...
      if(confirmResult !== 'confirm') {
        ...
        const {data} = await removeRole(id)
        if(!data) return
        if(data.meta.status === 200) {
    ...
    // 删除角色权限
    async removeRightById(role, item) {
      ...
      const {data} = await removeRoleRight(role.id, item.id, item.level)
      if(!data) return
      if(data.meta.status !== 200) return this.$message.error('删除角色权限失败')
      role.children = data.data.result.children
    ...
    async showSetRight(role){
      ...
      if(!data) return
      if(data.meta.status !== 200) return this.$message.error('获取权限信息失败')
      this.rightsList = data.data.result
    ...
    async allotRights() {
      ...
      const {data} = await setRights(this.roleId, keys)
      if(!data) return
      if(data.meta.status !== 200) return this.$message.error('设置用户权限失败')
